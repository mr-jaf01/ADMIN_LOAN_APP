...
Class Action {
	...
	function save_loan_type(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT IGNORE INTO loan_types set ".$data);
		}else{
	...
	function save_plan(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT IGNORE INTO loan_plan set ".$data);
		}else{
	...
	function save_borrower(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT IGNORE INTO borrowers set ".$data);
		}else{
	...
	function save_loan(){
		extract($_POST);
			...
			if(isset($status)){
				...
				if($status == 2){
					...
					if($chk->num_rows > 0){
						...
						$this->db->query("INSERT IGNORE INTO loan_schedules set loan_id = $id, date_due ='$date' ");
			...
			if(empty($id))
			$save = $this->db->query("INSERT IGNORE INTO loan_list set ".$data);
			else
	...
	function save_payment(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT IGNORE INTO payments set ".$data);
		}else{
